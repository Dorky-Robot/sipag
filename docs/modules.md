...
- Bridge dispatcher is a pipeline of pure functions + one async LLM call: `(window: &[KatulongEvent], ctx: &KrContext) → render_prompt → llm_client.call → parse_action → dispatch_to_recording_api`. The pure halves (`render_prompt`, `parse_action`) are unit-testable without a live ollama, matching the `dispatch_gate::render_user_prompt` + `dispatch_gate::parse_decision` template that's already proven out (see `sipag/src/dispatch_gate.rs` — moved out of sipag-core in §9 #9 — for the live example).
- Bridge (and every lens-worker) takes a **`ChatBackend`** trait (this section's `dyn LlmClient` framing predates the lens-worker abstraction; `sipag_lens::ChatBackend` is the trait the framing points at). Tests inject a mock that returns canned structured-JSON responses; production wires `BridgeChatBackend` which routes through `ollama-bridge-client`. This seam was successfully reused by the dispatch gate flow when it folded in §9 #9 — the gate now talks gemma through the same `ChatBackend` (path: `sipag/src/dispatch_gate.rs`). (`nudge.rs` was the obvious sibling here when this section was written; it retired in §9 #11 since the WS-attach path doesn't need post-dispatch keystroke retry.) **Each lens-worker gets its own `LlmClient` instance configured for its model** rather than a single global one — that's how per-lens model choice composes through the test seam.
...
| ~~`sipag-core/src/gate.rs`~~ | ✅ moved to `sipag/src/dispatch_gate.rs` in §9 #9 | now calls gemma via `sipag_lens::ChatBackend` — the wire is unified even though the gate kept its one-shot calling convention (output shape doesn't fit a `StructuralAction` verb) |
...
3. **✅ DONE — Lens-worker abstraction + Corpus + corpus-search tools + bridge as first worker** (third rewrite — see 2026-05-17 reframes in §3). Experimentation. The biggest single piece of work; the foundation everything observation-related sits on. Shipped across 8 PRs: *substrate* — `sipag-corpus` (PR #550), `sipag-lens` (PR #551), `ollama-bridge-client` (PR #549); *tool wrappers* — `corpus.search` / `corpus.expand` + `LensWorker::run_with_tools` multi-turn loop (PR #556); *scheduler* — `sipag/src/serve/lens_scheduler.rs` (PR #558, `--lens-scheduler` flag); *SSE subscriber* — `katulong-client/src/sse.rs` (PR #562); *bridge worker* — `sipag/src/serve/bridge_worker.rs` (PR #564, `--bridge-worker` flag); *session discovery wiring* — observer → bridge worker (PR #565). Enable end-to-end: `sipag serve --bridge-worker --workers --lens-scheduler`. **What's pending as follow-ups** (not blocking #3's closure): structural-verb dispatch to UI surfaces (`suggest_stance` / `ask_human` / `propose_task` are produced but logged at warn, not rendered into the KR sidebar); blind-spot trigger (§3 second-tier: deferred until per-fire outcome tracking exists); startup session discovery from katulong metadata (deferred until `TmuxSession` wire type carries Claude metadata); `~/.sipag/models.toml` Profile → concrete model mapping (defaults baked in, config file optional). Introduce:
...
7. **✅ DONE — katulong-client SSE subscriber + bridge consumer + proxy retirement.** Topology. Third wire surface alongside WS attach + HTTP. Built against today's `claude/<uuid>` topic; gains `sessions/<id>/*` for free when katulong#715/#716 land. Feeds the bridge lens-worker (Phase 1 #3) — emits structured `KatulongEvent`s that the bridge's sliding window consumes. Shipped across 4 PRs: subscriber surface (PR #562), bridge worker consuming the stream (PR #564), observer → bridge worker session discovery wiring (PR #565), `sipag sub` migrated to native SSE + Demeter-violating transcript proxy deleted (PR #567). The old `claude_transcript_url` URL builder, `observation_transcript_handler`, `fetch_recent_transcript`, and the `FeedEntry`/`FeedTool` type graph are all gone (net -277 lines in PR #567). Per `[[feedback-strict-layer-coupling]]`, sipag no longer reaches through katulong to Claude's transcript JSONL.
...
9. **✅ DONE — Fold gate into the lens-worker abstraction.** Experimentation. The gate moved out of `sipag-core/src/gate.rs` to `sipag/src/dispatch_gate.rs` and now talks gemma through `sipag_lens::ChatBackend` (concretely `BridgeChatBackend`) instead of the legacy direct-reqwest `llm::chat` path. The wire is unified across the dispatch gate, the lens scheduler, and any future bridge consumer. The gate kept its one-shot calling convention because `GateDecision { status_name, reason, human_action }` doesn't fit any of the four `StructuralAction` verbs cleanly — the fold is about the wire, not the call shape. `sipag-core` shrinks by one module (only `gate.rs` retired here; `llm.rs` stays for now since `categorize.rs` / `workers::research` / `workers::expand` still depend on it). (`nudge.rs` previously sat alongside as the post-dispatch sibling; retired in §9 #11.)
...
- 2026-05-27 — **§9 Phase 2 #7 fully closed (PR #567).** `sipag sub` migrated from curl shell-out to native `katulong_client::sse::subscribe()` + single-threaded tokio runtime. Same CLI interface, same output; removes the curl dependency and the api-key-in-ps-output exposure. Demeter-violating transcript proxy deleted: `claude_transcript_url` URL builder + test removed from `katulong-client`; `observation_transcript_handler` replaced with a placeholder; `fetch_recent_transcript` + `fetch_feeds` + `FeedEntry` / `FeedTool` / `TranscriptResponse` / `transcript_panel` / `transcript_row` + the `feeds` field on `BoardSnapshot` — all deleted. Net -277 lines. The transcript tab shows a placeholder message; a corpus-backed view is a follow-up UI task. §9 #7 is now fully closed alongside #3 (previous entry).
- 2026-05-27 — **§9 Phase 1 #3 closed + Phase 2 #7 bridge consumer landed (PRs #564 + #565).** Bridge lens-worker shipped in `sipag/src/serve/bridge_worker.rs` (PR #564): per-session reactive observer that subscribes to katulong `claude/<uuid>` SSE topics, maintains a 200-event sliding window (`SessionWindow`), fires gemma via `LensWorker::run_with_tools` on threshold (every 10 events), writes observations to the shared corpus (`Arc<Mutex<Corpus>>` opened once at startup for both scheduler + bridge worker), logs structural verbs at warn. `BridgeHandle` + unbounded channel so the observer can feed topics at runtime. New `--bridge-worker` CLI flag on `sipag serve`. Built-in bridge lens definition (`Profile::Fast`, `TriggerPolicy::Threshold`, hardcoded system prompt describing the 5 action types). Observer → bridge worker session discovery wiring (PR #565): `upsert_observation` in `observers.rs` calls `bridge_handle.watch(format!("claude/{uuid}"))` on every poll where the UUID is present (always-send + internal-dedup eliminates the startup race where the observer's first poll fires before the bridge handle is written). `is_valid_session_id()` validates the UUID before topic construction for defense-in-depth. 7 unit tests cover trigger logic, window bounds, prompt rendering, and lens shape. Deferred follow-ups: structural-verb dispatch to UI surfaces; blind-spot trigger; startup session discovery from katulong metadata; exponential backoff on reconnect; coordinator task reaping. End-to-end: `sipag serve --bridge-worker --workers --lens-scheduler`. Docs updated: CLAUDE.md (src tree, serve flags, overview), feature-matrix.md (6 rows), modules.md §9 #3 + #7.
- 2026-05-26 — **§9 Phase 2 #7 partially landed: katulong-client SSE subscriber surface.** Added `katulong-client/src/sse.rs` with `KatulongEvent` (typed `seq` + `event` + `timestamp` + optional `session` core; per-event-type fields flatten into `extra`), `KatulongEventStream` (async `Stream<Item = Result<KatulongEvent, SseError>>`), and `subscribe(http, base, api_key, topic, from_seq)`. Hand-rolled SSE line parser to avoid adding a new SSE crate (the existing `bytes` transitive dep of reqwest is now surfaced directly so the stream type can name `bytes::Bytes`); tolerant to comment / heartbeat lines per the SSE spec; bounded connect-error body (1KB cap) for log discipline. 6 feature-requirement tests use a hand-rolled tokio TcpListener-backed SSE server (no axum-test / wiremock dev-dep needed) and pin: two-back-to-back events parse correctly with `extra` populated, comment lines drop silently, malformed JSON surfaces as `SseError::BadEvent` (caller-skippable), non-2xx connect surfaces as `SseError::Connect` with truncated body, URL builder percent-encodes `/` in topic to one path segment, `from_seq` threads through, trailing slash on base is normalized. What's still pending in #7: the bridge lens-worker concrete instance that consumes the stream (Phase 1 #3 remainder), migrating `sipag/src/cli.rs::run_sub` from its curl-shelling impl to the native subscriber (deferred for risk isolation), and retiring the Demeter-violating `claude_transcript_url` / `observation_transcript_handler` (tied to bridge worker landing).
- 2026-05-22 — **§9 Phase 2 #9 landed: gate folded into the lens-worker abstraction (the wire half).** `sipag-core/src/gate.rs` (346 LOC) moved to `sipag/src/dispatch_gate.rs`; the `pub mod gate;` declaration removed from `sipag-core/src/lib.rs`; `sipag-core` shrinks by one module. The gate now talks gemma via `sipag_lens::ChatBackend` (concretely `BridgeChatBackend`) instead of the legacy direct-reqwest `sipag_core::llm::chat` path. New `sipag/src/bridge.rs` extracts the shared `OllamaBridgeClient` + `BridgeChatBackend` + `BridgeEmbedder` construction that the scheduler also uses (DRY out the inline construction that lived in `serve/mod.rs::spawn_lens_scheduler`). `AppState` gains `bridge: Option<BridgeWiring>` populated at startup; the dispatch gate borrows from it, the scheduler takes a clone. `BridgeChatBackend` + `BridgeEmbedder` gained `#[derive(Clone)]` so `BridgeWiring` can derive `Clone` (their underlying `OllamaBridgeClient` was already `Clone`-friendly via Arc-internal reqwest::Client). Operator config migration: dispatch now requires `~/.ollama-bridge/remote.json`; missing config → gate path fails closed with a clear "configure the bridge" message rather than spinning at gemma-unreachable. `sipag-core/src/llm.rs` stays — `categorize.rs`, `workers::research`, `workers::expand` are the three remaining callers; they migrate in a follow-up. The gate kept its one-shot calling convention (`GateInput` → `GateDecision`) because the output shape doesn't fit any of the four `StructuralAction` verbs cleanly — the "fold" is about the wire, not the call shape. 11 tests cover the gate (4 new `classify`-via-`ChatBackend` tests + 7 moved verbatim from the old gate.rs). Docs: `CLAUDE.md` (src tree + "What changes most"), modules.md §6 + §8 + §9 #9, feature-matrix.md (two rows), extraction-plan.md (tree listing + summary).
