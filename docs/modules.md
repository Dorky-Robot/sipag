...
|---|---|---|---|
| `Session` (in `katulong-client::http`) | Topology | `TmuxSession` | ✅ done 2026-05-17 (PR follow-up to #536). `KatulongSession` rejected: the type literally models a tmux session; `katulong_client::TmuxSession` is already namespaced. |
| `Session` (in `sipag-core::auth::session`) | Identity | `AuthSession` | 🟡 deferred — module path (`auth::session::Session`) already disambiguates at call sites; revisit when touching auth |
...
2. **Naming disambiguation pass.** Cross-cutting. Mechanical rename per the §6 table — splitting overloaded `Session` and `Status` across contexts. Sized as multiple focused PRs (one per context) to keep diffs reviewable. **Partially landed 2026-05-17**: katulong-client's `Session` → `TmuxSession` and `SessionStatus` → `TmuxSessionStatus` done; auth's `Session` → `AuthSession` deferred (module path already disambiguates); all `Status` renames deferred until §10 domain-vs-schema-noun question resolves; `ClaudeSession` / `DispatchSession` are greenfield names for types that don't exist yet.
