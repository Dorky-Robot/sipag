...
7. **⚙️ PARTIALLY LANDED — katulong-client SSE subscriber.** Topology. Third wire surface alongside WS attach + HTTP. Built against today's `claude/<uuid>` topic; gains `sessions/<id>/*` for free when katulong#715/#716 land. Feeds the gemma4 bridge (Phase 1 #3) — emits structured `KatulongEvent`s that the bridge's sliding window consumes. **This is the canonical replacement for the existing Demeter-violating Claude-transcript-proxy path** (`katulong-client::http::claude_transcript_url` + `serve/htmx.rs::observation_transcript_handler`) — once #7 fully lands (subscriber consumed by the bridge worker), that path retires per `[[feedback-strict-layer-coupling]]`. **What shipped (2026-05-26)**: the subscriber crate-level surface in `katulong-client/src/sse.rs` — `KatulongEvent` (typed `seq` + `event` + `timestamp` + optional `session`, per-event-type fields flatten into `extra`), `KatulongEventStream` (async `Stream<Item = Result<KatulongEvent, SseError>>`), `subscribe(http, base, api_key, topic, from_seq)`. Hand-rolled SSE line parser (no new crate dep beyond `bytes`); tolerant to comment / heartbeat lines; bounded connect-error body for log discipline. 6 feature-requirement tests use a hand-rolled TcpListener-backed SSE server (no axum-test / wiremock dev-dep needed). **What's pending**: (a) the bridge lens-worker concrete instance that consumes the stream — that closes the Phase 1 #3 remainder, (b) migrating `sipag/src/cli.rs::run_sub` from its current curl-shelling implementation to the native subscriber (deferred to a separate PR for risk isolation), (c) retiring the Demeter-violating `claude_transcript_url` / `observation_transcript_handler` once the bridge consumes the SSE stream end-to-end.
