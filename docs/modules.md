...
| Experimentation: `ask_human(...)` action | Steering: KR sidebar `steering-question` item | The gemma4 bridge's `ask_human` recording API → surfaces in the KR sidebar (deduped per the per-kind windows in §3 failure-mode policy). Distinct from the `operator-alert` channel, which renders inside Experimentation's own UI and never crosses into Steering. |
