...
- **Operator-vs-steering signal separation** — the bridge has TWO escalation channels:
  - **Operator channel** (new — sidebar item with `kind: operator-alert`) for "sipag is misfiring" signals: parse-failure threshold, dispatch-rejection threshold, gemma-unreachable status. Audience is the human-as-sipag-operator.
  - **`ask_human` channel** (sidebar item with `kind: steering-question`) for genuine "the work needs your input" signals from the bridge's derivation. Audience is the human-as-steerer.
...
- Operator-channel and `ask_human` channel are concrete types — separate sinks under the hood, even if they share the pub/sub broker as the transport.
