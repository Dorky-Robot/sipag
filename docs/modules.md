...
3. **Lens-worker abstraction + Corpus + corpus-search tools + bridge as first worker** (third rewrite — see 2026-05-17 reframes in §3). Experimentation. The biggest single piece of work; the foundation everything observation-related sits on. **⚙️ PARTIALLY LANDED 2026-05-20**: the *substrate* shipped — `sipag-corpus` (PR #550, storage + Embedder trait + BridgeEmbedder), `sipag-lens` (PR #551, Lens + ModelChoice + four verbs + LensWorker runtime), `ollama-bridge-client` (PR #549, queue-aware wire client). What's still pending: the **scheduler loop** that fires lens-workers on schedule/threshold (TriggerPolicy types are stored on the lens; the dispatcher that consumes them lives in sipag/serve, follow-up), the **bridge lens-worker concrete instance** (the first reactive trigger on katulong `claude/<uuid>` events), and the **`corpus.search` / `corpus.expand` MCP tool wrappers** (the gemma-callable surface). Introduce:
