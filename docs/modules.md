...
7. **⚙️ PARTIALLY LANDED — katulong-client SSE subscriber + bridge consumer.** Topology. Third wire surface alongside WS attach + HTTP. Built against today's `claude/<uuid>` topic; gains `sessions/<id>/*` for free when katulong#715/#716 land. Feeds the bridge lens-worker (Phase 1 #3) — emits structured `KatulongEvent`s that the bridge's sliding window consumes. **This is the canonical replacement for the existing Demeter-violating Claude-transcript-proxy path** (`katulong-client::http::claude_transcript_url` + `serve/htmx.rs::observation_transcript_handler`) — the bridge now consumes the SSE stream end-to-end, so the transcript-proxy path can retire per `[[feedback-strict-layer-coupling]]`. **What shipped**: (PR #562, 2026-05-26) the subscriber crate-level surface in `katulong-client/src/sse.rs`; (PR #564, 2026-05-27) the bridge lens-worker consuming the stream; (PR #565, 2026-05-27) observer → bridge worker session discovery wiring. **What's pending**: (a) migrating `sipag/src/cli.rs::run_sub` from its current curl-shelling implementation to the native subscriber (deferred for risk isolation), (b) retiring the Demeter-violating `claude_transcript_url` / `observation_transcript_handler` (the bridge replaces its function; deletion is cleanup).
