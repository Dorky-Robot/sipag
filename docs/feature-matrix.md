...
| **`LensWorker` runtime** (lens text + `ModelChoice` + trigger policy + query strategy + write protocol) | 🔴 | not implemented | Phase 1 #3 core. Every Steering entry spawns a worker; plus project-meta + ad-hoc workers. The bridge is the first instance. **Each worker gets its own `LlmClient` configured for its model** — bridge tier `Fast`, derivation tier `Strong`, code-aware lenses `CodeAware`. See modules.md §3 `ModelChoice` / `Profile`. |
| **Per-lens model selection** (`ModelChoice` + `Profile` + `~/.sipag/models.toml`) | 🔴 | not implemented | Phase 1 #3. Decouples lens definitions from concrete model names; same lenses run on different machines with different `Profile` → model maps. Tentative defaults: Fast=gemma4:latest, Strong=gemma4:31b, CodeAware=qwen2.5-coder:7b. Cloud-only models excluded by default per strict-layer-coupling. |
