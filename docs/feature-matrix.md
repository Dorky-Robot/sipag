...
| **Bridge lens-worker** (reactive on katulong `claude/<uuid>` events; sliding window of 200) | 🔴 | not implemented | Phase 1 #3 remainder. Needs §9 Phase 2 #7 (SSE subscriber in katulong-client) OR a polling fallback. The lens-worker runtime + scheduler are ready; only the concrete instance + event source are pending. |
