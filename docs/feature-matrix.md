...
| ~~Claude transcript proxy~~ | ✅ | deleted (PR #567) | **Demeter violation retired.** `claude_transcript_url`, `observation_transcript_handler`, `fetch_recent_transcript`, `FeedEntry`/`FeedTool` — all deleted. The bridge lens-worker (PR #564) consumes `claude/<uuid>` SSE events directly. The transcript tab now shows a placeholder; a corpus-backed view is a follow-up UI task. Per `[[feedback-strict-layer-coupling]]`. |
