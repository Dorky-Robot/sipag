...
sipag is pure scaffolding. Claude Code is the intelligence. This document explains the boundary.
...
- **Lifecycle**: Tracks worker state (enqueued, running, done, failed), manages labels, detects stale heartbeats
...
The worker prompt (`lib/prompts/worker-grouped.md`) encodes a **procedure** — not just instructions, but a specific analysis framework:
