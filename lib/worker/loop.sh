...
#
# Orchestrates the polling cycle: reconcile merged PRs, fix conflicted PRs
# (merge main forward — never rebase), dispatch PR iteration workers, dispatch
# new issue workers. Calls into github.sh, dedup.sh, and docker.sh.
#
...
worker_loop() {
    ...
    while true; do
        ...
        if [[ ${#new_issues[@]} -eq 0 && ${#prs_to_iterate[@]} -eq 0 && ${#prs_to_merge_forward[@]} -eq 0 ]]; then
