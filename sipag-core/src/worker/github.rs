...
/// Check whether an open or merged PR already exists that references the issue.
///
/// Searches for "Closes #N", "Fixes #N", or "Resolves #N" in open and merged
/// PR bodies. Using the PR body search avoids relying on branch-name prefix
/// matching (e.g. `--head "sipag/issue-3-*"` does not work as a glob in `gh`).
pub fn has_pr_for_issue(repo: &str, issue_num: u64) -> bool {
    let search = format!("closes #{issue_num}");
    let jq = format!(
        r#"[.[] | select(
            (.body // "" | ascii_downcase | test("(closes|fixes|resolves) #{issue_num}(\\b|[^0-9])")) and
