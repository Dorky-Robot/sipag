...
mod tests {
    ...
    fn extract_fixes_keyword() {
        assert_eq!(extract_issue_num_from_body("Fixes #7"), Some(7));
    }

    #[test]
    fn extract_resolves_keyword() {
        assert_eq!(extract_issue_num_from_body("Resolves #100"), Some(100));
    }

    #[test]
    fn extract_case_insensitive() {
        assert_eq!(extract_issue_num_from_body("CLOSES #5"), Some(5));
        assert_eq!(extract_issue_num_from_body("closes #5"), Some(5));
        assert_eq!(extract_issue_num_from_body("Closes #5"), Some(5));
    }

    #[test]
    fn extract_from_multiline_body() {
        let body = "This PR adds a feature.\n\nCloses #309\n\nSome more text.";
        assert_eq!(extract_issue_num_from_body(body), Some(309));
    }

    #[test]
    fn extract_returns_first_match() {
        let body = "Closes #1\nFixes #2";
        assert_eq!(extract_issue_num_from_body(body), Some(1));
    }

    #[test]
    fn extract_returns_none_when_no_match() {
        assert_eq!(extract_issue_num_from_body("No issue reference here."), None);
    }

    #[test]
    fn extract_returns_none_for_empty_body() {
        assert_eq!(extract_issue_num_from_body(""), None);
    }

    #[test]
    fn extract_does_not_match_partial_keyword() {
        // "unclosed #3" shouldn't match
        assert_eq!(extract_issue_num_from_body("unclosed #3"), None);
    }

    #[test]
    fn extract_large_issue_number() {
        assert_eq!(
            extract_issue_num_from_body("Closes #999999"),
            Some(999_999)
        );
    }

    // ── slug truncation (via slugify + take(50)) ──────────────────────────────
    // This tests the pattern used in dispatch_issue_worker to build branch names.

    #[test]
    fn slug_truncation_is_char_safe() {
        use crate::task::slugify;

        // A title that produces a slug longer than 50 characters.
        let title = "This is a very long issue title that exceeds fifty characters total";
        let slug = slugify(title);
        let truncated: String = slug.chars().take(50).collect();

        // Result must be a valid UTF-8 string (no panic), and at most 50 chars.
        assert!(truncated.chars().count() <= 50);
        // Must not end mid-char (char boundary safe).
        assert!(truncated.is_char_boundary(truncated.len()));
    }

    #[test]
    fn slug_truncation_with_ascii_only() {
        use crate::task::slugify;

        let slug = slugify("Fix authentication bug in login flow with token refresh");
        let truncated: String = slug.chars().take(50).collect();
        assert!(truncated.chars().count() <= 50);
    }
}
